# res://addons/quest_weaver/ui/quest_weaver_graph.gd
@tool
...
func update_node_structure_and_connections(node_id: String) -> void:
	...
	# Schritt 1: Aktualisieren Sie die interne Struktur des Knotens. Das ist sicher.
	_build_node_internal_structure(visual_node, node_data)
	if visual_node is QWGraphNode:
		visual_node.summary_text = node_data.get_editor_summary()
		visual_node.queue_redraw()
	...
	# Schritt 2: Anstatt manuell Verbindungen zu ändern, überlassen wir es der
	# zuverlässigen Funktion, die alle Verbindungen basierend auf den aktuellen
	# Daten neu zeichnet. `call_deferred` stellt sicher, dass der Knoten
	# zuerst seine Port-Änderungen verarbeiten kann.
...
func _gui_input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.is_released():
		# We use call_deferred to ensure that GraphEdit has fully processed
		# the selection before we check it.
...
func _build_node_internal_structure(visual_node: GraphNode, node_data: GraphNodeResource):
	for child in visual_node.get_children():
		if child is Container: child.queue_free()
	...
	for i in range(port_row_count):
		...
		# Erstelle die Port-Labels und graue sie bei Bedarf aus.
...
func _create_visual_node(node_data: GraphNodeResource, is_part_of_full_rebuild: bool = false) -> GraphElement:
	...
	# Node creation logic (this part was correct)
	if node_data is BackdropNodeResource:
		var frame = GraphFrame.new()
		frame.mouse_filter = Control.MOUSE_FILTER_PASS
		frame.size = node_data.node_size
		frame.resizable = true
		frame.resize_end.connect(func(_new_size: Vector2): node_data.node_size = frame.size)
		visual_node = frame
		...
		g_node.resize_end.connect(func(_new_size: Vector2): node_data.node_size = g_node.size)
...
func _apply_node_decorations(visual_node: GraphElement, node_data: GraphNodeResource) -> void:
	...
	# --- Logic for all other nodes (GraphNode) ---
	if visual_node is GraphNode:
		visual_node.title = node_registry.get_name_for_script(node_data.get_script())
		_build_node_internal_structure(visual_node, node_data)
